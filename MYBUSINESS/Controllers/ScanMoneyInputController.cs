...
namespace MYBUSINESS.Controllers
{
    public class ScanMoneyInputController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            var shopmanage = db.ScanMoneyInputs.OrderByDescending(p => p.Id) // Sorting by Id in descending order
                           .ToList().ToList();
        ...
        public ActionResult Create()
        {
            ...
            decimal maxId = db.ScanMoneyInputs.DefaultIfEmpty().Max(p => p == null ? 0 : p.Id);
            maxId += 1;
            ...
            ScanMoneyInputViewModel scanMoneyInputViewModel = new ScanMoneyInputViewModel
            {
                ScanMoneyInput = DAL.dbScanMoneyInput.FirstOrDefault(),
            ...
            return View(scanMoneyInputViewModel); // Passing the ViewModel to the view
        }
        ...
        public ActionResult Create([Bind(Include = "Id,StoreId,Note")] ScanMoneyInput scanmoneyInput)

        {
            if (ModelState.IsValid)
            {

                var shop = db.ScanMoneyInputs.FirstOrDefault(p => p.Id == scanmoneyInput.StoreId);


                db.ScanMoneyInputs.Add(scanmoneyInput);
        ...
        public ActionResult Edit(decimal id)
        {
            ...
            ScanMoneyInput scanMoneyInput = db.ScanMoneyInputs.Find(id);
            if (scanMoneyInput == null)
            {
                return HttpNotFound();
            }
        ...
        .ToList();
            ...
            ViewBag.StoreList = new SelectList(stores, "Value", "Text", scanMoneyInput.StoreId.ToString());


            var scanMoneyInputViewModel = new ScanMoneyInputViewModel
            {
                ScanMoneyInput = scanMoneyInput,
                Products = DAL.dbProducts,

            };

            return View(scanMoneyInputViewModel);
        }
        ...
        [ValidateAntiForgeryToken]
        [ValidateInput(false)]
        public ActionResult Edit([Bind(Include = "Id,StoreId,Note")] ScanMoneyInput scanmoneyInput)
        {
            if (ModelState.IsValid)
            {
                // Update ScanBankDeposit
                db.Entry(scanmoneyInput).State = EntityState.Modified;
                db.SaveChanges();
