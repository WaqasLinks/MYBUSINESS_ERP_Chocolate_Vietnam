...
namespace MYBUSINESS.Controllers
{
    ...
    public class PrintedPackagingReciverController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            IQueryable<PPReceiver> pOes = db.PPReceivers
.Where(x => x.Date >= dtStartDate && x.Date <= dtEndtDate && x.SupplierId > 0)
.Include(s => s.Supplier)
.Include(p => p.PPReciverDetails);
            ...
            int thisSerial = 0;
            foreach (PPReceiver itm in pOes)
            {
        ...
        public ActionResult SearchData(string suppId, string startDate, string endDate)
        {
            ...
            DateTime dtEndtDate;
            IQueryable<PPReceiver> selectedPOes = null;
            ...
            if (suppId != string.Empty && startDate != string.Empty && endDate != string.Empty)
            {
                ...
                selectedPOes = db.PPReceivers.Where(so => so.SupplierId == intSuppId && so.Date >= dtStartDate && so.Date <= dtEndtDate);
            ...
            if (suppId == string.Empty && startDate != string.Empty && endDate != string.Empty)
            {
                ...
                selectedPOes = db.PPReceivers.Where(so => so.Date >= dtStartDate && so.Date <= dtEndtDate);
            ...
            Dictionary<decimal, decimal> LstMaxSerialNo = new Dictionary<decimal, decimal>();
            int thisSerial = 0;
            foreach (PPReceiver itm in selectedPOes)
            {
                thisSerial = (int)itm.Supplier.POes.Max(x => x.POSerial);
                if (!LstMaxSerialNo.ContainsKey((int)itm.SupplierId))
                {
        ...
        {
            IQueryable<PPReceiver> pOes = db.PPReceivers.Include(s => s.Supplier);

            //sOes = db.SOes.Where(x => x.SODs.Where(y => y.ProductId == productId));

            List<PPReciverDetail> lstPODs = db.PPReciverDetails.Where(x => x.ProductId == productId && x.SaleType == false).ToList();
            List<PPReceiver> lstSlectedPO = new List<PPReceiver>();
            foreach (PPReciverDetail lpod in lstPODs)
            {
                if (lstSlectedPO.Where(x => x.Id == lpod.PPReciverId).FirstOrDefault() == null)
                {
                    lstSlectedPO.Add(lpod.PPReceiver);
                }
            }
        ...
        public ActionResult SearchProduct(int productId)
        {
            ...
            List<PPReciverDetail> lstPODs = db.PPReciverDetails.Where(x => x.ProductId == productId).ToList();
            List<PPReceiver> lstSlectedPO = new List<PPReceiver>();
            foreach (PPReciverDetail lpod in lstPODs)
            {
                if (lstSlectedPO.Where(x => x.Id == lpod.PPReciverId).FirstOrDefault() == null)
        ...
        private void GetTotalBalance(ref IQueryable<PPReceiver> POes)
        {
            //IQueryable<SO> DistSOes = SOes.Select(x => x.CustomerId).Distinct();
            IQueryable<PPReceiver> DistPOes = POes.GroupBy(x => x.SupplierId).Select(y => y.FirstOrDefault());

            decimal TotalBalance = 0;
            foreach (PPReceiver itm in DistPOes)
            {
        ...
        public ActionResult Details(decimal id)
        {
            ...
            PPReceiver pO = db.PPReceivers.Find(id);
            if (pO == null)
            {
                return HttpNotFound();
            }
        ...
        new MyUnitType { Text = "Pack", Value = "true" }
            ...
            PPReceiver pO = db.PPReceivers.Find(iid);
            if (pO == null)
            {
                return HttpNotFound();
            }

            List<PPReciverDetail> pod = db.PPReciverDetails.Where(x => x.PPReciverId == iid).ToList();

            PrintedPackagingReciverViewModel printedPackagingReciverViewModel = new PrintedPackagingReciverViewModel
            {
            ...
            printedPackagingReciverViewModel.PPReceiver = pO;
            ...
            return View(printedPackagingReciverViewModel);
        }
        ...
        public ActionResult Create([Bind(Prefix = "Supplier", Include = "Name,Address")] Supplier Supplier, [Bind(Prefix = "PPReceiver", Include = "BillAmount,Balance,PrevBalance,BillPaid,Discount,SupplierId,Remarks,Remarks2,PaymentMethod,PaymentDetail,PurchaseReturn,FundingSourceId,BankAccountId,Date")] PPReceiver pOReciver, [Bind(Prefix = "PurchaseReciverOrderDetail", Include = "ProductId,Quantity,SaleType,PerPack,IsPack,PurchasePrice,ExpiryDate,PurchasingDate,Unit,QtyReceived")] List<PPReciverDetail> pODRecivers)
        {
            //PO pO = new PO();
            if (ModelState.IsValid)
            {
                ...
                decimal maxId1 = (int)db.PPReceivers.DefaultIfEmpty().Max(p => p == null ? 0 : p.POSerial);
                maxId1 += 1;
                ...
                db.PPReceivers.Add(pOReciver);
                //db.SaveChanges();
                int sno = 0;
                if (pODRecivers != null)
                {
                    //pOD.RemoveAll(so => so.ProductId == null);
                    foreach (PPReciverDetail pod in pODRecivers)
                    {
                        sno += 1;
                        pod.PPDReciverId = sno;
                        //pod.PO = pOReciver;
                        pod.PPReciverId = pOReciver.Id;
                    ...
                    db.PPReciverDetails.AddRange(pODRecivers);
                }
                db.SaveChanges();
            ...
            //return View(pO);
            PrintedPackagingReciverViewModel printedpackagingReciverViewModel = new PrintedPackagingReciverViewModel();
            printedpackagingReciverViewModel.Suppliers = DAL.dbSuppliers;
            printedpackagingReciverViewModel.Products = DAL.dbProducts.Where(x => x.PType == 4 || x.PType == 7 && x.IsService == false);
            return View(printedpackagingReciverViewModel);
            //return View();
        ...
        public ActionResult Edit(PrintedPackagingReciverViewModel printedpackagingReciverViewModel)
        {
            PPReceiver newPOReciver = printedpackagingReciverViewModel.PPReceiver;
            List<PPReciverDetail> newPODReciver = printedpackagingReciverViewModel.PPReciverDetail;
            if (ModelState.IsValid)
            {
                int? storeId = Session["StoreId"] as int?;
                //var storeId = Session["StoreId"] as string;
                ...
                PPReceiver PPReceiver = db.PPReceivers.Where(x => x.Id == newPOReciver.Id).FirstOrDefault();
                ...
                {//its means new supplier(not in db)
                 //PO.SupplierId = 10;
                 //int maxId = db.Suppliers.Max(p => p.Id);
                    supplier = printedpackagingReciverViewModel.Supplier;
                    decimal maxId = db.Suppliers.DefaultIfEmpty().Max(p => p == null ? 0 : p.Id);
                    maxId += 1;
                ...
                if (PPReceiver.SupplierId != newPOReciver.SupplierId)
                {//POme other db supplier
                 //first revert the previous supplier balance 
                    Supplier oldSupplier = db.Suppliers.Where(x => x.Id == PPReceiver.SupplierId).FirstOrDefault();
                    oldSupplier.Balance = db.PORecivers.Where(x => x.Id == PPReceiver.Id).FirstOrDefault().PrevBalance;
                ...
                PPReceiver.SupplierId = newPOReciver.SupplierId;
                PPReceiver.Supplier = supplier;

                /////////////////////////////////////////////////////////////////////////////



                List<PPReciverDetail> oldPODRecivers = db.PPReciverDetails.Where(x => x.PPReciverId == newPOReciver.Id).ToList();
                ...
                db.PPReciverDetails.RemoveRange(oldPODRecivers);
                //////////////////////////////////////////////////////////////////////////////
                ...
                if (newPODReciver != null)
                {
                    ...
                    foreach (PPReciverDetail pod in newPODReciver)
                    {
                        sno += 1;
                        pod.PPDReciverId = sno;
                        pod.PPReceiver = PPReceiver;
                        pod.PPReciverId = PPReceiver.Id;
                    ...
                    db.Entry(PPReceiver).State = EntityState.Modified;
                    db.Entry(PPReceiver).Property(x => x.POSerial).IsModified = false;
                    db.PPReciverDetails.AddRange(newPODReciver);
        ...
        public ActionResult Edit(string id, bool? update, bool readonlyMode = false)
        {
            ...
            PrintedPackagingReciverViewModel printedPackagingReciverViewModel = new PrintedPackagingReciverViewModel();
            List<PPReciverDetail> podRecivers = db.PPReciverDetails.Where(x => x.PPReciverId == id).ToList();
            printedPackagingReciverViewModel.Products = DAL.dbProducts.Where(x => x.PType == 4 || x.PType == 7 && x.IsService == false);
            printedPackagingReciverViewModel.Suppliers = DAL.dbSuppliers;
            printedPackagingReciverViewModel.PPReciverDetail = podRecivers;
            printedPackagingReciverViewModel.PPReceiver = pO;
