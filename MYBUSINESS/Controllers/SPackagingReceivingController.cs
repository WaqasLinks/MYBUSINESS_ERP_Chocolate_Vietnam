...
namespace MYBUSINESS.Controllers
{
    ...
    public class SPackagingReceivingController : Controller
    {
        private BusinessContext db = new BusinessContext();
        private DAL DAL = new DAL();
        // GET: SPackagingReceiving
        public ActionResult Index()
        {
            ...
            IQueryable<SPackgingReceiver> pOes = db.SPackgingReceivers
.Where(x => x.Date >= dtStartDate && x.Date <= dtEndtDate && x.SupplierId > 0)
.Include(s => s.Supplier)
.Include(p => p.SPDReceivers);
            ...
            int thisSerial = 0;
            foreach (SPackgingReceiver itm in pOes)
            {
        ...
        {
            IQueryable<SPackaging> pOes = db.SPackagings.Include(s => s.Supplier);
            ...
            foreach (SPackaging itm in pOes)
            {
                //itm.Id = Encryption.Encrypt(itm.Id, "BZNS");
                itm.Id = string.Join("-", ASCIIEncoding.ASCII.GetBytes(Encryption.Encrypt(itm.Id, "BZNS")));
            }
        ...
        private void GetTotalBalance(ref IQueryable<SPackaging> POes)
        {
            //IQueryable<SO> DistSOes = SOes.Select(x => x.CustomerId).Distinct();
            IQueryable<SPackaging> DistPOes = POes.GroupBy(x => x.SupplierId).Select(y => y.FirstOrDefault());

            decimal TotalBalance = 0;
            foreach (SPackaging itm in DistPOes)
            {
        ...
        new MyUnitType { Text = "Pack", Value = "true" }
            ...
            SPackaging pO = db.SPackagings.Find(iid);
            if (pO == null)
            {
                return HttpNotFound();
            }

            List<SPackagingDetail> pod = db.SPackagingDetails.Where(x => x.SPackagingId == iid).ToList();

            SPackagingReceiverViewModel spackagingreceiverViewModel = new SPackagingReceiverViewModel
            {
                Products = DAL.dbProducts.Where(x => (x.PType == 4 || x.PType == 7) && !x.IsService),
                Suppliers = DAL.dbSuppliers,
                SPackagingDetail = pod
            };

            pO.Id = Encryption.Encrypt(pO.Id, "BZNS");
            spackagingreceiverViewModel.SPackaging = pO;
            ...
            return View(spackagingreceiverViewModel);
        }
        ...
        public ActionResult Create([Bind(Prefix = "Supplier", Include = "Name,Address")] Supplier Supplier, [Bind(Prefix = "SPackgingReceiver", Include = "BillAmount,Balance,PrevBalance,BillPaid,Discount,SupplierId,Remarks,Remarks2,PaymentMethod,PaymentDetail,PurchaseReturn,FundingSourceId,BankAccountId,Date")] SPackgingReceiver pOReciver, [Bind(Prefix = "SPDReceiver", Include = "ProductId,Quantity,SaleType,PerPack,IsPack,PurchasePrice,ExpiryDate,PurchasingDate,Unit,QtyReceived")] List<SPDReceiver> pODRecivers)
        {
            //PO pO = new PO();
            if (ModelState.IsValid)
            {
                ...
                db.SPackgingReceivers.Add(pOReciver);
                //db.SaveChanges();
                int sno = 0;
                if (pODRecivers != null)
                {
                    //pOD.RemoveAll(so => so.ProductId == null);
                    foreach (SPDReceiver pod in pODRecivers)
                    {
                        sno += 1;
                    ...
                    db.SPDReceivers.AddRange(pODRecivers);
                }
                db.SaveChanges();
        ...
        public FileContentResult PrintSO3(string id)
        {
            ...
            int POSerial = (int)db.SPackagings.FirstOrDefault(x => x.Id == id).POSerial;
            // Variables
            Warning[] warnings;
            string[] streamIds;
        ...
        {
            IQueryable lstSO = db.SPackagings.Where(x => x.SupplierId == id);
        ...
        public ActionResult Edit(SPackagingReceiverViewModel spackagingreceiverViewModel1)
        {
            SPackgingReceiver newPOReciver = spackagingreceiverViewModel1.SPackgingReceiver;
            List<SPDReceiver> newPODReciver = spackagingreceiverViewModel1.SPDReceiver;
            if (ModelState.IsValid)
            {
                int? storeId = Session["StoreId"] as int?;
                //var storeId = Session["StoreId"] as string;
                ...
                SPackgingReceiver POReciver = db.SPackgingReceivers.Where(x => x.Id == newPOReciver.Id).FirstOrDefault();
                ...
                {//its means new supplier(not in db)
                 //PO.SupplierId = 10;
                 //int maxId = db.Suppliers.Max(p => p.Id);
                    supplier = spackagingreceiverViewModel1.Supplier;
                    decimal maxId = db.Suppliers.DefaultIfEmpty().Max(p => p == null ? 0 : p.Id);
                    maxId += 1;
                ...
                {//POme other db supplier
                 //first revert the previous supplier balance 
                    ...
                    oldSupplier.Balance = db.SPackgingReceivers.Where(x => x.Id == POReciver.Id).FirstOrDefault().PrevBalance;
                ...
                List<SPDReceiver> oldPODRecivers = db.SPDReceivers.Where(x => x.POReciverId == newPOReciver.Id).ToList();
                ...
                db.SPDReceivers.RemoveRange(oldPODRecivers);
                //////////////////////////////////////////////////////////////////////////////
                ...
                if (newPODReciver != null)
                {
                    ...
                    foreach (SPDReceiver pod in newPODReciver)
                    {
                        sno += 1;
                        pod.PODReciverId = sno;
                        pod.SPackgingReceiver = POReciver;
                        pod.POReciverId = POReciver.Id;
                    ...
                    db.SPDReceivers.AddRange(newPODReciver);
        ...
        public ActionResult Edit(string id, bool? update, bool readonlyMode = false)
        {
            ...
            SPackgingReceiver pO = db.SPackgingReceivers.Find(id);
            if (pO == null)
            {
                return HttpNotFound();
            }
            ...
            SPackagingReceiverViewModel spackagingreceiverViewModel = new SPackagingReceiverViewModel();
            //List<> podRecivers = db.SPDReceivers.Where(x => x.POReciverId == id).ToList();
            List<SPDReceiver> podRecivers = db.SPDReceivers.Where(x => x.POReciverId == id).ToList();

            spackagingreceiverViewModel.Products = DAL.dbProducts.Where(x => x.PType == 4 || x.PType == 7 && x.IsService == false);
            spackagingreceiverViewModel.Suppliers = DAL.dbSuppliers;
            spackagingreceiverViewModel.SPDReceiver = podRecivers;
            spackagingreceiverViewModel.SPackgingReceiver = pO;
        ...
        public JsonResult Validation(List<MYBUSINESS.Models.POPRViewModel> LstProductionVM)
        {
            try
            {
                ...
                foreach (var item in LstProductionVM)
                {
                    ...
                    var podRecord = db.SPDReceivers.FirstOrDefault(p =>
        ...
        public JsonResult GetLastOrderProducts(int supplierId)
        {
            ...
            var products = db.SPackagingDetails
     .Where(pod => pod.SPackagingId == lastOrder.Id)
     .Select(pod => new
     {
