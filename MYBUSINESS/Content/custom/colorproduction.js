...
var _total = 0;
var IsReturn = "false";
// Serialize the Model data to make it available in JavaScript
...
$(document).ready(function () {
    ...
    //    // Loop through all colors and build columns dynamically
    //    let colorInputs = "";
    //    for (let i = 0; i < colorList.length; i++) {
    //        let color = colorList[i];
    //        let planner = plannerQuantities[color] || 0;    

    //        colorInputs += '<td>' +
    //            '<input type="text" class="form-control qty-input" ' +
    //            'name="CPReceipts[' + txtSerialNum + '].ColorQuantities[' + i + '].QuantityReceived" ' +
    //            'data-index="' + txtSerialNum + '-' + i + '" ' +
    //            'data-color="' + color + '" ' +
    ...
    //    var row = '<tr>' +
    //        '<td>' + ($('#selectedProducts tr').length) + '</td>' +
    //        '<td style="display:none;"><input type="hidden" name="SubItem.Index" value="' + txtSerialNum + '" /></td>' +
    //        '<td><input type="datetime-local" class="form-control from-date-picker"  name="CPReceipts[' + txtSerialNum + '].ReceivedDate"></td>' +
    //        colorInputs +
    //        '</tr>';
    ...
    $("#addNewRow").click(function () {
        ...
        colorList.forEach((color, i) => {
            // Find first record for this color to get IDs
            const colorData = modelData.find(m => m.ColorName === color) || {};

            newRow += `<td>
            <input type="hidden" 
                   name="CPReceipts[${newRowIndex}].Items[${i}].PaperColorId" 
                   value="${colorData.PaperColorId || 0}" />
            <input type="hidden" 
                   name="CPReceipts[${newRowIndex}].Items[${i}].PPSubItemId" 
                   value="${colorData.PPSubItemId || 0}" />
            <input type="hidden" 
                   name="CPReceipts[${newRowIndex}].Items[${i}].PackagingProductionId" 
                   value="${colorData.PackagingProductionId || 0}" />
            <input type="hidden" 
                   name="CPReceipts[${newRowIndex}].Items[${i}].ColorName" 
                   value="${color}" />
